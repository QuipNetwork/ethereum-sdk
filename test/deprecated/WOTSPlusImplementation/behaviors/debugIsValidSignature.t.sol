...
contract WOTSPlusImplementation_debugIsValidSignature is WOTSPlusImplementationTest {
    ...
    function test_debugIsValidSignature_returnsBadSignatureLengthOnWrongLength() public {
        _seedVerificationKeys(1);
        bytes32 msgHash = keccak256("erc1271-debug-len");

        IWOTSPlusImplementation.Erc1271ValidationResult expected =
        IWOTSPlusImplementation.Erc1271ValidationResult.BadSignatureLength;

        assertEq(uint8(wallet.debugIsValidSignature(msgHash, bytes(""))), uint8(expected));
        assertEq(uint8(wallet.debugIsValidSignature(msgHash, new bytes(100))), uint8(expected));
        assertEq(uint8(wallet.debugIsValidSignature(msgHash, new bytes(2208))), uint8(expected));
        assertEq(uint8(wallet.debugIsValidSignature(msgHash, new bytes(2272))), uint8(expected));
        assertEq(uint8(wallet.debugIsValidSignature(msgHash, new bytes(2274))), uint8(expected));
    }

    function test_debugIsValidSignature_returnsInvalidEcdsaSignatureOnNonOwner() public {
        ...
        assertEq(
            uint8(wallet.debugIsValidSignature(msgHash, Codec.encodeErc1271Signature(keys[0], sig, ecdsa))),
            uint8(IWOTSPlusImplementation.Erc1271ValidationResult.InvalidEcdsaSignature)
        );
    }

    function test_debugIsValidSignature_returnsInvalidEcdsaSignatureOnWrongHash() public {
        (WOTSPlus.WinternitzAddress[] memory keys, bytes32[] memory priv) = _seedVerificationKeys(2);

        bytes32 msgHash = keccak256("erc1271-debug-ecdsa-wrong-hash");
        bytes32 digest = _buildErc1271MessageHash(address(wallet), keys[0], msgHash);
        WOTSPlus.WinternitzElements memory sig = _sign(priv[0], digest);
        bytes memory ecdsa = _ecdsaSign(ALICE_KEY, keccak256("not-the-actual-hash"));
    ...
    function test_debugIsValidSignature_returnsInvalidEcdsaSignatureOnMalformedEcdsa() public {
        (WOTSPlus.WinternitzAddress[] memory keys, bytes32[] memory priv) = _seedVerificationKeys(2);
    ...
    function test_debugIsValidSignature_returnsUnknownVerifierWhenVerifierNotInSet() public {
        _seedVerificationKeys(2);
        (WOTSPlus.WinternitzAddress memory outsider, bytes32 outsiderKey) = _generateKeyPair("erc1271-debug-outsider");
        ...
        assertEq(
            uint8(wallet.debugIsValidSignature(msgHash, Codec.encodeErc1271Signature(outsider, sig, ecdsa))),
            uint8(IWOTSPlusImplementation.Erc1271ValidationResult.UnknownVerifier)
        );
    }

    function test_debugIsValidSignature_returnsInvalidPqSignatureOnBadWotsSig() public {
        ...
        assertEq(
            uint8(wallet.debugIsValidSignature(msgHash, Codec.encodeErc1271Signature(keys[0], bad, ecdsa))),
            uint8(IWOTSPlusImplementation.Erc1271ValidationResult.InvalidPqSignature)
        );
    }
    ...
    function test_debugIsValidSignature_returnsInvalidPqSignatureOnWrongWalletBinding() public {
        (WOTSPlus.WinternitzAddress[] memory keys, bytes32[] memory priv) = _seedVerificationKeys(2);
    ...
    function test_debugIsValidSignature_returnsInvalidPqSignatureOnWrongMessageHash() public {
        (WOTSPlus.WinternitzAddress[] memory keys, bytes32[] memory priv) = _seedVerificationKeys(2);

        bytes32 digest = _buildErc1271MessageHash(address(wallet), keys[0], keccak256("debug-hash-A"));
        WOTSPlus.WinternitzElements memory sig = _sign(priv[0], digest);
        bytes memory ecdsa = _ecdsaSign(ALICE_KEY, _buildErc1271EcdsaTarget(address(wallet), keccak256("debug-hash-B")));

        assertEq(
            uint8(
                wallet.debugIsValidSignature(
                    keccak256("debug-hash-B"), Codec.encodeErc1271Signature(keys[0], sig, ecdsa)
                )
            ),
            uint8(IWOTSPlusImplementation.Erc1271ValidationResult.InvalidPqSignature)
        );
    ...
    function test_debugIsValidSignature_zeroPayloadShortCircuitsAtEcdsa() public view {
