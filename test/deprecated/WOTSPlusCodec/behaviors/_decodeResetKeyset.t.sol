...
contract WOTSPlusCodec__decodeResetKeyset is WOTSPlusCodecTest {
    function test_exposed_decodeResetKeyset_decodesTxSignTxTarget() public view {
        bytes memory payload = _buildResetKeysetPayload(42, Codec.KeyType.Transaction, Codec.KeyType.Transaction);
        assertEq(payload.length, 2976);
        WOTSPlusCodecHarness.DecodedResetKeyset memory out = codec.exposed_decodeResetKeyset(payload);
    ...
    function test_exposed_decodeResetKeyset_decodesRecSignVerifyTarget() public view {
        bytes memory payload = _buildResetKeysetPayload(7, Codec.KeyType.Verification, Codec.KeyType.Recovery);
        WOTSPlusCodecHarness.DecodedResetKeyset memory out = codec.exposed_decodeResetKeyset(payload);
    ...
    function test_exposed_decodeResetKeyset_decodesTxSignRecoveryTarget() public view {
        bytes memory payload = _buildResetKeysetPayload(99, Codec.KeyType.Recovery, Codec.KeyType.Transaction);
        WOTSPlusCodecHarness.DecodedResetKeyset memory out = codec.exposed_decodeResetKeyset(payload);
    ...
    function test_exposed_decodeResetKeyset_revertsWhen_shortPayload() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(Codec.MalformedPayload.selector, 2976, 100));
    ...
    function test_exposed_decodeResetKeyset_revertsWhen_oneByteLong() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(Codec.MalformedPayload.selector, 2976, 2977));
        codec.exposed_decodeResetKeyset(payload);
    }

    function test_exposed_decodeResetKeyset_revertsWhen_garbageSuffix() public {
        bytes memory payload = _buildResetKeysetPayload(30, Codec.KeyType.Recovery, Codec.KeyType.Transaction);
        payload = abi.encodePacked(payload, bytes32(uint256(0xDEAD)));
        vm.expectRevert(abi.encodeWithSelector(Codec.MalformedPayload.selector, 2976, payload.length));
        codec.exposed_decodeResetKeyset(payload);
    }

    function test_exposed_decodeResetKeyset_revertsWhen_kindOutOfRange() public {
        bytes memory payload = _buildResetKeysetPayload(30, Codec.KeyType.Recovery, Codec.KeyType.Transaction);
    ...
    function test_exposed_decodeResetKeyset_revertsWhen_signingKindOutOfRange() public {
        bytes memory payload = _buildResetKeysetPayload(30, Codec.KeyType.Recovery, Codec.KeyType.Transaction);
    ...
    function testFuzz_exposed_decodeResetKeyset_revertsWhen_wrongLength(uint256 len) public {
        len = bound(len, 0, 10000);
        vm.assume(len != 2976);
        vm.expectRevert(abi.encodeWithSelector(Codec.MalformedPayload.selector, 2976, len));
