...
contract WOTSPlusCodec__decodeOwnershipTransfer is WOTSPlusCodecTest {
    function _buildOwnershipTransferPayload(uint256 seed, address newOwner)
        internal
        pure
        returns (bytes memory payload)
    {
        ...
        payload = abi.encodePacked(payload, bytes32(uint256(uint160(newOwner))));
        // newDisasterKey (64)
        payload = abi.encodePacked(payload, bytes32(seed + 500), bytes32(seed + 501));
        // newTransactionKeys[10] (640)
        for (uint256 i = 0; i < 10; i++) {
            payload = abi.encodePacked(payload, bytes32(seed + 600 + i * 2), bytes32(seed + 601 + i * 2));
        }
        // newRecoveryKeys[10] (640)
        for (uint256 i = 0; i < 10; i++) {
            payload = abi.encodePacked(payload, bytes32(seed + 700 + i * 2), bytes32(seed + 701 + i * 2));
        }
        // newVerificationKeys[10] (640)
        for (uint256 i = 0; i < 10; i++) {
            payload = abi.encodePacked(payload, bytes32(seed + 800 + i * 2), bytes32(seed + 801 + i * 2));
        }
    }
    ...
    function test_exposed_decodeOwnershipTransfer_decodesCorrectly() public view {
        ...
        for (uint256 i = 0; i < 10; i++) {
            assertEq(newTxn[i].publicSeed, bytes32(uint256(10 + 600 + i * 2)));
            assertEq(newTxn[i].publicKeyHash, bytes32(uint256(10 + 601 + i * 2)));
        }
        for (uint256 i = 0; i < 10; i++) {
            assertEq(newRec[i].publicSeed, bytes32(uint256(10 + 700 + i * 2)));
            assertEq(newRec[i].publicKeyHash, bytes32(uint256(10 + 701 + i * 2)));
        }
        for (uint256 i = 0; i < 10; i++) {
            assertEq(newVer[i].publicSeed, bytes32(uint256(10 + 800 + i * 2)));
            assertEq(newVer[i].publicKeyHash, bytes32(uint256(10 + 801 + i * 2)));
        }
    }

    function test_exposed_decodeOwnershipTransfer_revertsWhen_emptyPayload() public {
        vm.expectRevert(abi.encodeWithSelector(WOTSPlusCodec.MalformedPayload.selector, 4288, 0));
        codec.exposed_decodeOwnershipTransfer("");
    }

    function test_exposed_decodeOwnershipTransfer_revertsWhen_wrongLength() public {
        vm.expectRevert(abi.encodeWithSelector(WOTSPlusCodec.MalformedPayload.selector, 4288, 4287));
        codec.exposed_decodeOwnershipTransfer(_filledBytes(4287));
    }

    /// @dev Property: any payload length other than 4288 reverts.
    function testFuzz_exposed_decodeOwnershipTransfer_revertsWhen_wrongLength(uint256 len) public {
        len = bound(len, 0, 7000);
        vm.assume(len != 4288);
        vm.expectRevert(abi.encodeWithSelector(WOTSPlusCodec.MalformedPayload.selector, 4288, len));
