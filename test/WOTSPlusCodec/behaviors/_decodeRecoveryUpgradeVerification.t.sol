...
contract WOTSPlusCodec__decodeRecoveryUpgradeVerification is
    WOTSPlusCodecTest
{
    ...
    function test_exposed_decodeRecoveryUpgradeVerification_decodesVerifier()
        public
        view
    {
        bytes memory payload = _buildRecoveryUpgradePayload(7);
        (WOTSPlus.WinternitzAddress memory v, ) = codec
            .exposed_decodeRecoveryUpgradeVerification(payload);
        assertEq(v.publicSeed, bytes32(uint256(7 + 2000)));
        assertEq(v.publicKeyHash, bytes32(uint256(7 + 2001)));
    }

    function test_exposed_decodeRecoveryUpgradeVerification_decodesVerifySig()
        public
        view
    {
        bytes memory payload = _buildRecoveryUpgradePayload(7);
    ...
    function test_exposed_decodeRecoveryUpgradeVerification_revertsWhen_emptyPayload()
        public
    {
        vm.expectRevert(
    ...
    function test_exposed_decodeRecoveryUpgradeVerification_revertsWhen_wrongLength()
        public
    {
        vm.expectRevert(
            abi.encodeWithSelector(
