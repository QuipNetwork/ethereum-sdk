...
contract WOTSPlusCodec__upgradeRecoveryDigest is WOTSPlusCodecTest {
    ...
    function test_exposed_upgradeRecoveryDigest_matchesManualHash() public view {
        ...
        assertEq(codec.exposed_upgradeRecoveryDigest(W, C, impl, S1, H1, S2, H2), expected);
    }

    function test_exposed_upgradeRecoveryDigest_differsByImplementation() public view {
        bytes32 a = codec.exposed_upgradeRecoveryDigest(W, C, address(0x1), S1, H1, S2, H2);
        bytes32 b = codec.exposed_upgradeRecoveryDigest(W, C, address(0x2), S1, H1, S2, H2);
        assertTrue(a != b);
    }

    function test_exposed_upgradeRecoveryDigest_differsFromUpgradeDigest() public view {
        bytes32 a = codec.exposed_upgradeRecoveryDigest(W, C, address(0xDEAD), S1, H1, S2, H2);
