...
contract WOTSPlusCodec__decodeSaveWallet is WOTSPlusCodecTest {
    ...
    function _buildSaveWalletPayload(uint256 seed) internal pure returns (bytes memory payload) {
        ...
        for (uint256 i = 0; i < 10; i++) {
            payload = abi.encodePacked(payload, bytes32(seed + 500 + i * 2), bytes32(seed + 501 + i * 2));
        }
        for (uint256 i = 0; i < 10; i++) {
            payload = abi.encodePacked(payload, bytes32(seed + 700 + i * 2), bytes32(seed + 701 + i * 2));
        }
        for (uint256 i = 0; i < 10; i++) {
            payload = abi.encodePacked(payload, bytes32(seed + 900 + i * 2), bytes32(seed + 901 + i * 2));
        }
    }
    ...
    function test_exposed_decodeSaveWallet_decodesCorrectly() public view {
        ...
        for (uint256 i = 0; i < 10; i++) {
            ...
            assertEq(newTxn[i].publicKeyHash, bytes32(uint256(42 + 501 + i * 2)));
            assertEq(newRec[i].publicSeed, bytes32(uint256(42 + 700 + i * 2)));
            assertEq(newRec[i].publicKeyHash, bytes32(uint256(42 + 701 + i * 2)));
            assertEq(newVer[i].publicSeed, bytes32(uint256(42 + 900 + i * 2)));
            assertEq(newVer[i].publicKeyHash, bytes32(uint256(42 + 901 + i * 2)));
        }
    }
    ...
    function test_exposed_decodeSaveWallet_revertsWhen_extraBytes() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(WOTSPlusCodec.MalformedPayload.selector, 4192, 4224));
        codec.exposed_decodeSaveWallet(payload);
    }

    function test_exposed_decodeSaveWallet_revertsWhen_emptyPayload() public {
        vm.expectRevert(abi.encodeWithSelector(WOTSPlusCodec.MalformedPayload.selector, 4192, 0));
        codec.exposed_decodeSaveWallet("");
    }

    function test_exposed_decodeSaveWallet_revertsWhen_truncatedPayload() public {
        vm.expectRevert(abi.encodeWithSelector(WOTSPlusCodec.MalformedPayload.selector, 4192, 3000));
        codec.exposed_decodeSaveWallet(_filledBytes(3000));
    }

    /// @dev Property: any payload length other than 4192 reverts.
    function testFuzz_exposed_decodeSaveWallet_revertsWhen_wrongLength(uint256 len) public {
        len = bound(len, 0, 6000);
        vm.assume(len != 4192);
        vm.expectRevert(abi.encodeWithSelector(WOTSPlusCodec.MalformedPayload.selector, 4192, len));
