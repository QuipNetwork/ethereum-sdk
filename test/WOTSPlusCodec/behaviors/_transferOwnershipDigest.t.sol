...
contract WOTSPlusCodec__transferOwnershipDigest is WOTSPlusCodecTest {
    ...
    function test_exposed_transferOwnershipDigest_matchesManualHash() public view {
        ...
        assertEq(codec.exposed_transferOwnershipDigest(W, C, S1, H1, S2, H2, newOwner), expected);
    }

    function test_exposed_transferOwnershipDigest_differsByNewOwner() public view {
        bytes32 a = codec.exposed_transferOwnershipDigest(W, C, S1, H1, S2, H2, address(0x1));
        bytes32 b = codec.exposed_transferOwnershipDigest(W, C, S1, H1, S2, H2, address(0x2));
        assertTrue(a != b);
    }

    function test_exposed_transferOwnershipDigest_differsByWallet() public view {
        bytes32 a = codec.exposed_transferOwnershipDigest(address(0x1), C, S1, H1, S2, H2, address(0xBEEF));
        bytes32 b = codec.exposed_transferOwnershipDigest(address(0x2), C, S1, H1, S2, H2, address(0xBEEF));
        assertTrue(a != b);
    }

    function test_exposed_transferOwnershipDigest_differsByChainId() public view {
        bytes32 a = codec.exposed_transferOwnershipDigest(W, 1, S1, H1, S2, H2, address(0xBEEF));
        bytes32 b = codec.exposed_transferOwnershipDigest(W, 2, S1, H1, S2, H2, address(0xBEEF));
        assertTrue(a != b);
    }
}
