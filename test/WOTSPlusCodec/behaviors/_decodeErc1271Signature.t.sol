...
contract WOTSPlusCodec__decodeErc1271Signature is WOTSPlusCodecTest {
    ...
    function _buildErc1271Payload(uint256 seed) internal pure returns (bytes memory payload) {
    ...
    function test_exposed_decodeErc1271Signature_decodesVerifier() public view {
        ...
        (WOTSPlus.WinternitzAddress memory v,,) = codec.exposed_decodeErc1271Signature(payload);
    ...
    function test_exposed_decodeErc1271Signature_decodesPqSig() public view {
        bytes memory payload = _buildErc1271Payload(42);
        (, WOTSPlus.WinternitzElements memory sig,) = codec.exposed_decodeErc1271Signature(payload);
        for (uint256 i = 0; i < 67; i++) {
    ...
    function test_exposed_decodeErc1271Signature_revertsWhen_truncatedPayload() public {
        vm.expectRevert(abi.encodeWithSelector(WOTSPlusCodec.MalformedPayload.selector, 2273, 2200));
        codec.exposed_decodeErc1271Signature(_filledBytes(2200));
    }

    function test_exposed_decodeErc1271Signature_revertsWhen_extraBytes() public {
        vm.expectRevert(abi.encodeWithSelector(WOTSPlusCodec.MalformedPayload.selector, 2273, 2274));
    ...
    function testFuzz_exposed_decodeErc1271Signature_revertsWhen_wrongLength(uint256 len) public {
        len = bound(len, 0, 5000);
        vm.assume(len != 2273);
        vm.expectRevert(abi.encodeWithSelector(WOTSPlusCodec.MalformedPayload.selector, 2273, len));
