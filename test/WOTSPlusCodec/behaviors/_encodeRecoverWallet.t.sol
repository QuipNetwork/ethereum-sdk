...
contract WOTSPlusCodec__encodeRecoverWallet is WOTSPlusCodecTest {
    function test_exposed_encodeRecoverWallet_producesCorrectLength() public view {
        WOTSPlus.WinternitzAddress memory rk = WOTSPlus.WinternitzAddress(bytes32(uint256(1)), bytes32(uint256(2)));
        WOTSPlus.WinternitzAddress memory pq = WOTSPlus.WinternitzAddress(bytes32(uint256(3)), bytes32(uint256(4)));
    ...
    function test_exposed_encodeRecoverWallet_roundtrips() public view {
        ...
        for (uint256 i = 0; i < 67; i++) assertEq(dSig.elements[i], sig.elements[i]);
    }
}
