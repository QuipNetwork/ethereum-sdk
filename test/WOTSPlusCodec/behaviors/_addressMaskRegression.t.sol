...
contract WOTSPlusCodec_addressMaskRegression is Test {
    ...
    function _buildWithdrawPayload(
        bytes32 addressWord,
        uint256 amount
    ) internal pure returns (bytes memory payload) {
    ...
    function test_decodeExecute_returnsAddressForCleanWord() public view {
        ...
        bytes memory payload = _buildExecutePayload(
            _stuffAddressWord(0, target),
            42
        );
        (, , , address decoded, uint256 value, ) = harness.exposed_decodeExecute(
            payload
        );
    ...
    function test_decodeExecute_masksUpper96BitsAllOnes() public view {
        ...
        bytes memory payload = _buildExecutePayload(
            _stuffAddressWord(type(uint96).max, target),
            7
        );
    ...
    function test_decodeExecute_masksUpper96BitsSingleHighBit() public view {
        ...
        bytes memory payload = _buildExecutePayload(
            _stuffAddressWord(garbage, target),
            0
        );
        (, , , address decoded, , ) = harness.exposed_decodeExecute(payload);
        assertEq(decoded, target);
    ...
    function testFuzz_decodeExecute_masksAnyUpperGarbage(
        uint96 garbage,
        address target,
        uint256 value
    ) public view {
        ...
        (, , , address decoded, uint256 v, ) = harness.exposed_decodeExecute(
            payload
        );
    ...
    function test_decodeWithdrawDeposit_masksUpper96BitsAllOnes() public view {
        ...
        bytes memory payload = _buildWithdrawPayload(
            _stuffAddressWord(type(uint96).max, to),
            1_000_000
        );
        (, , , address decoded, uint256 amount) = harness
            .exposed_decodeWithdrawDeposit(payload);
        assertEq(decoded, to);
        assertEq(amount, 1_000_000);
    }

    function testFuzz_decodeWithdrawDeposit_masksAnyUpperGarbage(
        uint96 garbage,
        address to,
        uint256 amount
    ) public view {
        bytes memory payload = _buildWithdrawPayload(
            _stuffAddressWord(garbage, to),
            amount
        );
        (, , , address decoded, uint256 a) = harness
            .exposed_decodeWithdrawDeposit(payload);
        assertEq(decoded, to);
        assertEq(a, amount);
    }
    ...
    function test_decodeOwnershipTransfer_masksUpper96BitsAllOnes()
        public
        view
    {
        address newOwner = address(uint160(0xFEED_BEEF_C0DE_BABE));
        bytes memory payload = _buildOwnershipTransferPayload(
            _stuffAddressWord(type(uint96).max, newOwner)
        );
        (, , , address decoded, , , , ) = harness
            .exposed_decodeOwnershipTransfer(payload);
        assertEq(decoded, newOwner);
    }

    function testFuzz_decodeOwnershipTransfer_masksAnyUpperGarbage(
        uint96 garbage,
        address newOwner
    ) public view {
        bytes memory payload = _buildOwnershipTransferPayload(
            _stuffAddressWord(garbage, newOwner)
        );
        (, , , address decoded, , , , ) = harness
            .exposed_decodeOwnershipTransfer(payload);
        assertEq(decoded, newOwner);
    ...
    function testFuzz_decodeExecute_lowerTwentyBytesAreLoadBearing(
        uint256 fullWord,
        uint256 value
    ) public view {
