...
contract WOTSPlusCodec__executeDigest is WOTSPlusCodecTest {
    ...
    function test_exposed_executeDigest_matchesManualHash() public view {
        ...
        bytes32 expected = EfficientHashLib.hash(
            ...
            S1, H1, S2, H2,
            bytes32(uint256(uint160(target))), bytes32(value), opHash, bytes32(fee)
        );
        assertEq(codec.exposed_executeDigest(W, C, S1, H1, S2, H2, target, value, opHash, fee), expected);
    }

    function test_exposed_executeDigest_differsByTargetOrValue() public view {
        bytes32 opHash = bytes32(uint256(99));
        bytes32 a = codec.exposed_executeDigest(W, C, S1, H1, S2, H2, address(0x1), 1 ether, opHash, 0);
        bytes32 b = codec.exposed_executeDigest(W, C, S1, H1, S2, H2, address(0x2), 1 ether, opHash, 0);
        bytes32 c = codec.exposed_executeDigest(W, C, S1, H1, S2, H2, address(0x1), 2 ether, opHash, 0);
        assertTrue(a != b);
        assertTrue(a != c);
    }
    ...
    function test_exposed_executeDigest_differsByFee() public view {
        bytes32 opHash = bytes32(uint256(99));
        bytes32 a = codec.exposed_executeDigest(W, C, S1, H1, S2, H2, address(0x1), 1 ether, opHash, 0);
        bytes32 b = codec.exposed_executeDigest(W, C, S1, H1, S2, H2, address(0x1), 1 ether, opHash, 0.01 ether);
        assertTrue(a != b);
    }
}
