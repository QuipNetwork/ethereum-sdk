...
contract WOTSPlusCodec__encodeExecute is WOTSPlusCodecTest {
    function test_exposed_encodeExecute_producesCorrectLength() public view {
        ...
        bytes memory encoded = codec.exposed_encodeExecute(cur, nxt, sig, address(0xBEEF), 1 ether, data);
    ...
    function test_exposed_encodeExecute_roundtrips() public view {
        ...
        bytes memory encoded = codec.exposed_encodeExecute(cur, nxt, sig, target, value, data);
        (
    ...
    function test_exposed_encodeExecute_roundtripsEmptyData() public view {
        ...
        WOTSPlus.WinternitzAddress memory nxt = WOTSPlus.WinternitzAddress(bytes32(uint256(3)), bytes32(uint256(4)));
        WOTSPlus.WinternitzElements memory sig;
        bytes memory encoded = codec.exposed_encodeExecute(cur, nxt, sig, address(0x1), 0, "");
    ...
    function testFuzz_exposed_encodeExecute_roundtrips(bytes32 seed, address target, uint256 value, bytes calldata data)
        public
        view
    {
