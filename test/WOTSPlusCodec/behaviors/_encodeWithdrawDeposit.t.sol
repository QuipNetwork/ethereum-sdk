...
contract WOTSPlusCodec__encodeWithdrawDeposit is WOTSPlusCodecTest {
    function test_exposed_encodeWithdrawDeposit_producesCorrectLength() public view {
        ...
        bytes memory encoded = codec.exposed_encodeWithdrawDeposit(cur, nxt, sig, address(0xBEEF), 1 ether);
    ...
    function test_exposed_encodeWithdrawDeposit_roundtrips() public view {
        ...
        for (uint256 i = 0; i < 67; i++) {
            sig.elements[i] = bytes32(i + 100);
        }
        address to = address(0xCAFE);
        uint256 amount = 2.5 ether;

        bytes memory encoded = codec.exposed_encodeWithdrawDeposit(cur, nxt, sig, to, amount);
        (
    ...
    function testFuzz_exposed_encodeWithdrawDeposit_roundtrips(bytes32 seed, address to, uint256 amount) public view {
