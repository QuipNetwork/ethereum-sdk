...
contract QuipPaymaster_getDeposit is QuipPaymasterTest {
    function test_getDeposit_returnsEntryPointBalance() public {
        vm.mockCall(
            ENTRY_POINT, abi.encodeWithSignature("balanceOf(address)", address(paymaster)), abi.encode(uint256(5 ether))
        );
    ...
    function test_getDeposit_returnsZeroWhenNoDeposit() public {
        vm.mockCall(
            ENTRY_POINT, abi.encodeWithSignature("balanceOf(address)", address(paymaster)), abi.encode(uint256(0))
        );
