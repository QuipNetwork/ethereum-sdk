...
contract QuipPaymaster_verifierLifecycle is QuipPaymasterTest {
    function _validate(bytes memory data, address sender_) internal {
        ...
        paymaster.validatePaymasterUserOp(_mockUserOp(data, sender_), bytes32(0), 0);
    }

    function _validateExpectingFailure(bytes memory data, address sender_) internal returns (uint256 validationData) {
        vm.prank(ENTRY_POINT);
        (, validationData) = paymaster.validatePaymasterUserOp(_mockUserOp(data, sender_), bytes32(0), 0);
    }

    function test_simulation_verifierLifecycle() public {
        // ── Step 1: EntryPoint-driven rotation from the initial verifier ──
        (WOTSPlus.WinternitzAddress memory nextV, bytes32 nextVPriv) = _generateKeyPair("vlc-next-1");
        bytes memory data1 = _buildPaymasterAndData(
            WALLET, 0, "", uint48(block.timestamp + 1 hours), uint48(0), verifierPubkey, verifierPrivateKey, nextV
        );
        _validate(data1, WALLET);

        // Verifier rotated.
        assertEq(paymaster.getPqVerifier(WALLET).publicSeed, nextV.publicSeed);
        ...
        //   takes over.
        (WOTSPlus.WinternitzAddress memory forcedV, bytes32 forcedVPriv) = _generateKeyPair("vlc-forced");
        vm.prank(ADMIN);
        paymaster.setPqVerifier(WALLET, forcedV);
        assertEq(paymaster.getPqVerifier(WALLET).publicSeed, forcedV.publicSeed);
        ...
        {
            (WOTSPlus.WinternitzAddress memory stale,) = _generateKeyPair("vlc-stale-next");
        ...
        (WOTSPlus.WinternitzAddress memory afterForced,) = _generateKeyPair("vlc-after-forced");
        {
            ...
            bytes memory data2 = _buildPaymasterAndData(
                WALLET, 0, "", uint48(block.timestamp + 1 hours), uint48(0), forcedV, forcedVPriv, afterForced
            );
            _validate(data2, WALLET);
            assertEq(paymaster.getPqVerifier(WALLET).publicSeed, afterForced.publicSeed);
        }
        ...
        {
            (WOTSPlus.WinternitzAddress memory orphanNext,) = _generateKeyPair("vlc-orphan-next");
            bytes memory orphanData = _buildPaymasterAndData(
                WALLET, 0, "", uint48(block.timestamp + 1 hours), uint48(0), forcedV, forcedVPriv, orphanNext
            );
        ...
        (WOTSPlus.WinternitzAddress memory reregPub, bytes32 reregPriv) = _generateKeyPair("vlc-rereg");
        vm.prank(ADMIN);
        paymaster.setPqVerifier(WALLET, reregPub);
        assertEq(paymaster.getPqVerifier(WALLET).publicSeed, reregPub.publicSeed);

        // A UserOp under the freshly registered key validates and rotates.
        (WOTSPlus.WinternitzAddress memory postRereg,) = _generateKeyPair("vlc-post-rereg");
        bytes memory reregData = _buildPaymasterAndData(
            WALLET, 0, "", uint48(block.timestamp + 1 hours), uint48(0), reregPub, reregPriv, postRereg
        );
        _validate(reregData, WALLET);
        assertEq(paymaster.getPqVerifier(WALLET).publicSeed, postRereg.publicSeed);
    }
}
