...
contract QuipWallet_resetKeyset_Recovery is QuipWalletTest {
    ...
    function test_resetKeyset_revertsWhen_signingKindIsVerification() public {
        ...
        vm.prank(ALICE);
        vm.expectRevert(IQuipWallet.InvalidSigningKeyset.selector);
        harnessProxy.resetKeyset(payload);
    }

    function test_resetKeyset_revertsWhen_callerIsNotOwner() public {
        WOTSPlus.WinternitzAddress[10] memory newKeys = _freshKeys10(
            keccak256("rev-caller")
        );
        (WOTSPlus.WinternitzAddress memory nextPq, ) = _generateKeyPair(
            "rev-caller-next"
        );
    ...
    function test_resetKeyset_revertsWhen_currentKeyNotInSigningSet() public {
        ...
        (
            ...
            bytes32 currentPriv
        ) = _generateKeyPair("rev-unknown-current");
        WOTSPlus.WinternitzAddress[10] memory newKeys = _freshKeys10(
    ...
    function test_resetKeyset_revertsWhen_sameKeysetAuth_newKeysContainsCurrentKey()
        public
    {
    ...
    function test_resetKeyset_revertsWhen_sameKeysetAuth_newKeysContainsNextKey()
        public
    {
