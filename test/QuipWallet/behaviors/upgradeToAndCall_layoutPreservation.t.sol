...
///         `QuipWallet.sol` and `WOTSPlusStorage.sol` match their documented
...
///      `test/fixtures/`.
contract QuipWallet_upgradeToAndCall_layoutPreservation is QuipWalletTest {
    ...
    ///      `QuipWallet.sol` (`_PQ_FACTORY_SLOT` etc.) silently desync from the
