...
contract QuipWallet_addKeys_Verification is QuipWalletTest {
    function _genKeys(
        string memory tag,
        uint256 n
    ...
    function test_addKeys_Verification_addsKeys() public {
        ...
        vm.prank(ALICE);
        wallet.addKeys(Codec.encodeKeyManagement(Codec.KeyType.Verification, alicePubkey, nextPq, sig, newKeys)
        );

        assertEq(wallet.keyCount(Codec.KeyType.Verification), 3);
        for (uint256 i = 0; i < 3; i++) {
            assertTrue(wallet.isKey(Codec.KeyType.Verification, newKeys[i]));
        }
    }
    ...
    function test_addKeys_Verification_emitsKeysAdded() public {
    ...
    function test_addKeys_Verification_appendsToExisting() public {
        _seedVerificationKeys(2);
        assertEq(wallet.keyCount(Codec.KeyType.Verification), 2);
    ...
    function test_addKeys_Verification_revertsWhen_callerNotOwner() public {
    ...
    function test_addKeys_Verification_revertsWhen_invalidSignature() public {
        WOTSPlus.WinternitzAddress[] memory newKeys = _genKeys(
            "add-bad-sig",
            1
        );
        (WOTSPlus.WinternitzAddress memory nextPq, ) = _generateKeyPair(
        ...
        wallet.addKeys(Codec.encodeKeyManagement(Codec.KeyType.Verification, alicePubkey, nextPq, badSig, newKeys)
        );
    }
        ...
        public
    {
        ...
        vm.prank(ALICE);
        vm.expectRevert(IQuipWallet.InvalidSignature.selector);
        wallet.addKeys(Codec.encodeKeyManagement(Codec.KeyType.Verification, alicePubkey, zeroPq, sig, newKeys)
        );
    }
    ...
    function test_addKeys_Verification_revertsWhen_emptyArray() public {
        ...
        wallet.addKeys(Codec.encodeKeyManagement(Codec.KeyType.Verification, alicePubkey, nextPq, sig, empty)
        );
    }

    function test_addKeys_Verification_revertsWhen_capacityExceeded() public {
    ...
    function test_addKeys_Verification_revertsWhen_keyHasZeroSeed() public {
        WOTSPlus.WinternitzAddress[]
        ...
        wallet.addKeys(Codec.encodeKeyManagement(Codec.KeyType.Verification, alicePubkey, nextPq, sig, badKeys)
        );
    }

    function test_addKeys_Verification_revertsWhen_keyHasZeroHash() public {
        WOTSPlus.WinternitzAddress[]
            memory badKeys = new WOTSPlus.WinternitzAddress[](1);
        badKeys[0] = WOTSPlus.WinternitzAddress({
            publicSeed: bytes32("non-empty"),
        ...
        public
    {
        ...
        WOTSPlus.WinternitzAddress[]
            memory dup = new WOTSPlus.WinternitzAddress[](1);
