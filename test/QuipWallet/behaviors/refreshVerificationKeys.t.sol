...
contract QuipWallet_refreshVerificationKeys is QuipWalletTest {
    ...
    function _refresh(
        ...
        vm.prank(ALICE);
        wallet.refreshVerificationKeys(Codec.encodeKeyManagement(nextPq, sig, newKeys));
    }

    // ── Happy paths ──────────────────────────────────────────────────

    function test_refreshVerificationKeys_fromEmpty() public {
    ...
    function test_refreshVerificationKeys_fromPartial() public {
        (WOTSPlus.WinternitzAddress[] memory original,) = _seedVerificationKeys(2);
    ...
    function test_refreshVerificationKeys_fromFull() public {
        (WOTSPlus.WinternitzAddress[] memory original,) = _seedVerificationKeys(10);
    ...
    function test_refreshVerificationKeys_rotatesPqOwner() public {
    ...
    function test_refreshVerificationKeys_emitsEvent() public {
        ...
        vm.prank(ALICE);
        vm.recordLogs();
        wallet.refreshVerificationKeys(Codec.encodeKeyManagement(nextPq, sig, keys));

        Vm.Log[] memory logs = vm.getRecordedLogs();
        bool found;
        for (uint256 i = 0; i < logs.length; i++) {
            if (logs[i].topics[0] == IQuipWallet.VerificationKeysRefreshed.selector) {
                found = true;
                break;
            }
        }
        assertTrue(found, "VerificationKeysRefreshed not emitted");
    }

    // ── Reverts ──────────────────────────────────────────────────────

    function test_refreshVerificationKeys_revertsWhen_callerNotOwner() public {
    ...
    function test_refreshVerificationKeys_revertsWhen_invalidSignature() public {
        ...
        wallet.refreshVerificationKeys(Codec.encodeKeyManagement(nextPq, badSig, keys));
    }

    function test_refreshVerificationKeys_revertsWhen_nextPqOwnerSeedIsZero() public {
        ...
        wallet.refreshVerificationKeys(Codec.encodeKeyManagement(zeroPq, sig, keys));
    }

    function test_refreshVerificationKeys_revertsWhen_nextPqOwnerHashIsZero() public {
    ...
    function test_refreshVerificationKeys_revertsWhen_pqOwnerReuse() public {
        ...
        wallet.refreshVerificationKeys(Codec.encodeKeyManagement(alicePubkey, sig, keys));
    }

    function test_refreshVerificationKeys_revertsWhen_emptyArray() public {
        ...
        wallet.refreshVerificationKeys(Codec.encodeKeyManagement(nextPq, sig, empty));
    }

    function test_refreshVerificationKeys_revertsWhen_newBatchExceedsMax() public {
    ...
    function test_refreshVerificationKeys_revertsWhen_duplicateInBatch() public {
        ...
        wallet.refreshVerificationKeys(Codec.encodeKeyManagement(nextPq, sig, dup));
    }

    function test_refreshVerificationKeys_revertsWhen_keyHasZeroSeed() public {
        ...
        vm.prank(ALICE);
        vm.expectRevert(IQuipWallet.ZeroValuePqOwner.selector);
        wallet.refreshVerificationKeys(Codec.encodeKeyManagement(nextPq, sig, bad));
    }

    function test_refreshVerificationKeys_revertsWhen_keyHasZeroHash() public {
        WOTSPlus.WinternitzAddress[] memory bad = new WOTSPlus.WinternitzAddress[](1);
        bad[0] = WOTSPlus.WinternitzAddress({
            publicSeed: bytes32("x"),
            publicKeyHash: bytes32(0)
