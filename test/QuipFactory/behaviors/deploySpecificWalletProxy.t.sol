...
contract QuipFactory_deploySpecificWalletProxy is QuipFactoryTest {
    ...
    function test_deploySpecificWalletProxy_deploysWithBalance() public {
        ...
        (
            WOTSPlus.WinternitzAddress memory pubkey,
            bytes32 pk
        ) = _generateKeyPair("seed-specific");
        WOTSPlus.WinternitzAddress[] memory rKeys = _generateRecoveryKeys(
            pk,
            10
        );
        ...
        }(vaultId, 0, payable(ALICE), payload);
    ...
    function test_deploySpecificWalletProxy_emitsQuipCreatedEvent() public {
        ...
        for (uint256 i = 0; i < logs.length; i++) {
            if (
                logs[i].topics[0] ==
                keccak256(
                    "QuipCreated(uint256,uint256,bytes32,address,(bytes32,bytes32),address)"
                )
            ) {
                found = true;
                break;
    ...
    function test_deploySpecificWalletProxy_tracksVaultIds() public {
        ...
        (
            WOTSPlus.WinternitzAddress memory pubkey1,
            bytes32 privateKey1
        ) = _generateKeyPair("seed1");
        (
            WOTSPlus.WinternitzAddress memory pubkey2,
            bytes32 privateKey2
        ) = _generateKeyPair("seed2");
        WOTSPlus.WinternitzAddress[] memory rKeys1 = _generateRecoveryKeys(
            privateKey1,
            10
        );
        WOTSPlus.WinternitzAddress[] memory rKeys2 = _generateRecoveryKeys(
            privateKey2,
            10
        );
        ...
        factory.deploySpecificWalletProxy(
            ...
            0,
            payable(ALICE),
            _encodeInitPayload(pubkey1, rKeys1)
        );
        factory.deploySpecificWalletProxy(
            vaultId2,
            0,
            payable(ALICE),
            _encodeInitPayload(pubkey2, rKeys2)
        );
        vm.stopPrank();
    ...
    function test_deploySpecificWalletProxy_revertsWhen_duplicateVaultId()
        public
    {
    ...
    function test_deploySpecificWalletProxy_revertsWhen_sameVaultIdDifferentSenders()
        public
    {
        ...
        public
    {
        ...
        (
            WOTSPlus.WinternitzAddress memory pubkey,
            bytes32 pk
        ) = _generateKeyPair("specific-seed-zero");
        WOTSPlus.WinternitzAddress[] memory rKeys = _generateRecoveryKeys(
            pk,
            10
        );
