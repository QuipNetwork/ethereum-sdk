...
import {IEntryPoint, IEntryPointStake, PackedUserOperation} from
...
contract IntegrationBase is Test {
    ...
    function _encodeInitPayload(
        ...
        for (uint256 i = 0; i < recoveryKeys.length; i++) {
            payload = abi.encodePacked(
                payload, recoveryKeys[i].publicSeed, recoveryKeys[i].publicKeyHash
            );
        }
        return payload;
    }
    ...
    function _deployWalletStack() internal {
        ...
        bytes memory factoryBytecode =
            abi.encodePacked(type(QuipFactory).creationCode, abi.encode(ADMIN, 0.1 ether));
    ...
    function _deployPaymaster() internal {
        ...
        paymaster = QuipPaymaster(payable(
            CREATE3.deployDeterministic(proxyInitcode, keccak256("integration-paymaster"))
        ));
    ...
    function _signUserOp(
        ...
        userOp.signature = Codec.encodeUserOpSignature(nextPq, sig);
    }
}
