...
contract ShrincsE2E_multiOpOutOfOrder is ShrincsE2EBase {
    ...
    function test_e2e_outOfOrderLeavesAccepted() public {
        _handle(_op("outOfOrderA")); // nonce 0, leaves 3/3
        _handle(_op("outOfOrderB")); // nonce 1, leaves 2/2
    ...
    function test_e2e_staleWalletLeafRejected() public {
        _handle(_op("outOfOrderA")); // nonce 0 consumes wallet leaf 3
        _handle(_op("outOfOrderB")); // nonce 1 (advance the EntryPoint nonce to 2)

        PackedUserOperation memory stale = _op("staleWalletLeafReplay"); // nonce 2, reuses wallet leaf 3
