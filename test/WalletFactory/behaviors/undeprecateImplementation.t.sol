...
contract WalletFactory_undeprecateImplementation is WalletFactoryTest {
    // ── Happy paths ──────────────────────────────────────────────────
    ...
    function test_undeprecateImplementation_emitsImplementationUndeprecated() public {
        ...
        for (uint256 i = 0; i < logs.length; i++) {
            if (logs[i].topics[0] == IWalletFactory.ImplementationUndeprecated.selector) {
    ...
    function test_undeprecateImplementation_revertsWhen_notDeprecated() public {
        ...
        vm.prank(ADMIN);
        vm.expectRevert(IWalletFactory.NotDeprecated.selector);
        factory.undeprecateImplementation(address(walletImplementation));
    }
