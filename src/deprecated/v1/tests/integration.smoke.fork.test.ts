...
import { encodeInit, MAX_KEYS } from "../wotsCodec.js";
import {
  DEFAULT_ACCOUNT,
  deployErc1967Proxy,
  deployFactoryProxy,
...
maybeDescribe("Forked-mainnet smoke (FORK_RPC_URL set)", () => {
  test("end-to-end lifecycle: create wallet → execute → add keys → sponsored UserOp", async () => {
    ...
    const prepared = await client.prepareExecuteUserOp(zeroAddress, 0n, "0x");
    ...
    const paymasterPreStakeDrop =
      paymasterPreStakeBefore - paymasterPreStakeAfter;
    ...
    const unsponsored = await client.buildExecuteUserOp(zeroAddress, 0n, "0x");
