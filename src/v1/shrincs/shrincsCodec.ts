...
/// bytes32 pkSeed, PublicKey, uint32 hashSuite, bytes32 erc1271Commitment,
/// uint32 erc1271HashSuite)`. Head words land at `[0:32) commitment,
/// [32:64) pkSeed` by ABI order.
...
/// Invert `encodeInitPayload`: recover the main bundle from an install /
/// migrate payload. Malformed blobs throw `MalformedCodecPayloadError`
/// (same 0xc0-byte floor as on-chain `decodeInit`).
export function decodeInitPayload(payload: Hex): ShrincsPublicKey {
  try {
    const decoded = decodeAbiParameters(INIT_PAYLOAD_PARAMS, payload);
    const pk = decoded[2] as Parameters<typeof publicKeyFromAbi>[0];
    return publicKeyFromAbi(pk);
  } catch {
