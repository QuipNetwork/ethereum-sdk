...
async function checkWalletNonce(address) {
    const nonce = await hardhat_1.default.ethers.provider.getTransactionCount(address);
    const balance = await hardhat_1.default.ethers.provider.getBalance(address);
...
async function main() {
    ...
    const code = await hardhat_1.default.ethers.provider.getCode(process.env.DEPLOYER_ADDRESS);
    if (code !== "0x") {
        throw new Error(`Deployer contract already exists at ${process.env.DEPLOYER_ADDRESS} on this network`);
    }
    // Create wallet from private key
    const deployer = new hardhat_1.default.ethers.Wallet(process.env.DEPLOYER_PRIVATE_KEY, hardhat_1.default.ethers.provider);
    ...
    for (let i = curNonce; i <= EXPECTED_NONCE; i++) {
        ...
        const Deployer = await hardhat_1.default.ethers.getContractFactory("Deployer", deployer);
        ...
        console.log(`Total gas cost: ${hardhat_1.default.ethers.formatEther(deployGasFee)} ETH`);
    }
}
