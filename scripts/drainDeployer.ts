...
async function main() {
    ...
    if (nonce <= EXPECTED_NONCE) {
        throw new Error(`Cannot drain: Current nonce (${nonce}) is not greater than EXPECTED_NONCE (${EXPECTED_NONCE})`);
    }
    ...
    console.log(`Estimated Gas Cost (with 20% buffer): ${hre.ethers.formatEther(gasCost)} ETH`);
    ...
    console.log(`Final gas cost: ${hre.ethers.formatEther(receipt?.gasUsed! * receipt?.gasPrice!)} ETH`);
}
