...
function main(): void {
  ...
  // QuipFactory: WOTSPlus-linked creation bytecode WITHOUT ctor args. The
  ...
  writeRelease(factory, {
    ...
    comment:
      "QuipFactory linked creation bytecode (no constructor args) for CREATE3 deployment. Generated by scripts/release.ts from Foundry artifacts.",
  ...
  // QuipWallet: WOTSPlus-linked creation bytecode WITHOUT ctor args. The
  // constructor `(address payable factory_)` is technically canonical
  // (QuipFactory's CREATE3 address is invariant across chains) but we keep
  // the same "release = linked code only, deploy composes args" contract as
  // QuipFactory to avoid having a hidden divergence in which contracts have
  // baked-in args and which don't.
  const wallet = readArtifact("QuipWallet");
  writeRelease(wallet, {
    contractDir: "QuipWallet.sol",
    comment:
      "QuipWallet linked creation bytecode (no constructor args) for CREATE3 deployment. Generated by scripts/release.ts from Foundry artifacts.",
    ...
    linkedLibraries: { WOTSPlus: WOTS_ADDRESS },
    constructorArgsDescription:
      "constructor(address payable factory_). NOT encoded into this snapshot. Deploy scripts must abi.encode (address) the canonical QuipFactory CREATE3 address and append.",
  });
