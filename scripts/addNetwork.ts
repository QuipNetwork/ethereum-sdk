...
async function main() {
  ...
  if (code !== "0x") {
    throw new Error(
      `Deployer contract already exists at ${process.env.DEPLOYER_ADDRESS} on this network`
    );
  }
  ...
  if (curBalance < MIN_BALANCE) {
    throw new Error(
      `balance too low (${curBalance} ETH). Needs at least ${MIN_BALANCE} ETH.}`
    );
  }
  ...
  for (let i = curNonce; i <= EXPECTED_NONCE; i++) {
    ...
    if (
      ...
      console.error(
        `Deployer address mismatch. Expected ${process.env.DEPLOYER_ADDRESS} but got ${deployerAddress}`
      );
    }
