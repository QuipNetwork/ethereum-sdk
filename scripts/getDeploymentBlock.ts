...
async function main() {
    if (!process.env.CONTRACT_ADDRESS || !process.env.DEPLOYER_ADDRESS) {
        throw new Error("CONTRACT_ADDRESS and DEPLOYER_ADDRESS must be set in .env file");
    }
    ...
    if (deployEvent) {
        ...
        console.log(`${network.name},${deployEvent.blockNumber},${new Date(Number(block.timestamp) * 1000).toISOString()}`);
