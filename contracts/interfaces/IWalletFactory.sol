...
interface IWalletFactory {
    ...
    ///         returns `false` (i.e. `_commitments[oldOwner].remove(commitment)` or
    ///         `_commitments[newOwner].add(commitment)`). Indicates the per-owner
    ...
    /// @notice Returns the wallet address deployed at `commitment` on this
    ///         factory.
    /// @dev `commitment` is a GLOBAL CREATE3 salt — same commitment on every chain
    ///      resolves to the same deterministic address. The outer
    ///      `(owner, commitment) → wallet` shape from earlier versions was
    ///      misleading: the registry never gated lookup by owner, and the
    ///      address is a pure function of the salt. Off-chain integrators
    ///      MUST treat `commitment` as a first-come-first-served global resource.
    /// @param commitment The commitment used as CREATE3 salt.
    /// @return The wallet address, or `address(0)` if none deployed here.
    function wallets(bytes32 commitment) external view returns (address);
