...
- Those two never-revert properties are about signature CONTENT, not ABI framing. The codec bounds-checks only the TOP-LEVEL tail offsets of a blob (hard `MalformedPayload` revert on the transaction paths, `ok = false` on the ERC-1271 path). A NESTED tail offset (inside the `PublicKey`/`Signature` tails) that runs past `calldatasize` is caught by Solidity's own calldata accessor bounds check — a bare revert — at the first field read, i.e. `_leafIndex` or the `abi.encode(pk, sig)` re-encode, which execute in the caller's frame OUTSIDE the verifier try/catch. Policy per path:
  - ERC-4337 (`validateUserOp`, `validatePaymasterUserOp`): fail-closed hard revert, accepted. The EntryPoint wraps both calls in its own try/catch, so a revert (AA23/AA33) and a soft-fail (AA24/AA34) each reject exactly that one op; nothing is written, an op that fails first simulation never enters the mempool, and no ERC-7562 reputation counter moves. Reachable by anyone on the paymaster (no co-signature on the sponsorship blob) and, on the wallet, only by replaying a previously broadcast blob with its internals corrupted (the owner co-signature bytes are reusable).
  - ERC-1271 (`isValidSignature`): NOT accepted — a relying contract staticcalls it and a revert is a DoS on that contract. The nested reads run behind a self-staticcall (`erc1271Envelope`) whose revert maps to `MalformedErc1271Payload` / `0xffffffff`.
