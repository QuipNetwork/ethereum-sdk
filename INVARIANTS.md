...
- **Wallet paths and what they spend:** `initialize` and `migrate` spend all four trees — both halves of the main bundle AND both halves of the dedicated ERC-1271 bundle (main first, so a payload reusing the main bundle or either of its halves for the 1271 role trips on the 1271 bundle); `rotateKey` spends the next stateful tree (the stateless root is carried forward, already spent at install); `recoverWallet` and `transferOwnership` spend both trees of `nextKey`; `setErc1271Key` spends both trees of the new 1271 bundle before verifying the authorizing stateful signature. The paymaster's `initialize` and `rotateStatefulKey` spend the stateful tree (its stateless half is inert).
