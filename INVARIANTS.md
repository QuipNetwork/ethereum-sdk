...
- **Deploy shape:** impl + OZ `ERC1967Proxy` via sender-guarded CreateX CREATE3 (no in-repo Deployer hop), salt preimages `QUIP:WalletFactory:{Impl,Proxy}:V1.0.0-beta`. The fresh salts are load-bearing — CREATE3 ignores initcode, so the V1.1 salt would silently resolve to any pre-existing non-upgradeable factory deployment and skip. Sender-guarding is load-bearing too: the address is a function of (CreateX, `DEPLOY_OPERATOR`, preimage) and only the operator can consume the salt, so the canonical proxy address cannot be squatted on fresh chains.
