...
- Advances: +1 in `_verifyStatefulAndAdvance` and in the 4337 `_validateSignature` consume block (per consumed stateful signature); +1 in the `recoverWallet` / `transferOwnership` install blocks (per consumed stateless rotation signature). `transferOwnership` consumes one of each → nets +2, with both signatures binding the pre-call nonce (the rotation context is built and verified BEFORE the stateful core advances the nonce — load-bearing ordering). The incoming party's acceptance signatures (26) are the ONE exception to live-nonce binding: they bind nonce 0 / epoch 0 and advance nothing — their replay guard is the spent-tree registry (25), not the nonce.
...
- Those two never-revert properties cover ABI FRAMING as well as signature content on every validation path. The codec bounds-checks only the TOP-LEVEL tail offsets of a blob (hard `MalformedPayload` revert; `ok = false` from the ERC-1271 decoder). A NESTED tail offset (inside the `PublicKey`/`Signature` tails) that runs past `calldatasize` is caught by Solidity's own calldata accessor bounds check — a bare revert — at the first field read, i.e. `_leafIndex` or the `abi.encode(pk, sig)` re-encode, OUTSIDE the verifier try/catch. So on every validation path the codec decode, the leaf read and the re-encode run together behind a self-call-only STATICCALL whose revert the caller maps to the ordinary soft fail; no framing error can revert out of validation, and nothing is consumed:
  - ERC-4337 `validateUserOp`: `userOpEnvelope` → `validationData == 1` + `UserOpValidationFailure.MalformedSignature`. The framing check precedes the owner ECDSA gate (a corrupted blob is reported as malformed, not as a bad co-signature). The corrupted-blob case is reachable by replaying a previously broadcast blob with its internals corrupted (the owner co-signature bytes are reusable).
  - ERC-4337 `validatePaymasterUserOp`: `sponsorshipEnvelope` → `("", 1)` + `PaymasterValidationFailure.MalformedPayload`. Reachable by anyone (no co-signature on the sponsorship blob).
  - ERC-1271 `isValidSignature`: `erc1271Envelope` → `MalformedErc1271Payload` / `0xffffffff` — a relying contract staticcalls it and a revert would be a DoS on that contract.
  - Owner-gated transaction paths (`initialize`, the `upgradeToAndCall` auth blob, key rotation / recovery / handover): no wrapper. A malformed payload reverts the transaction (`MalformedPayload` or bare), the correct outcome for a signed transaction.
  - Why wrap the 4337 paths at all: the EntryPoint wraps both validation calls in its own try/catch, so a revert (AA23/AA33) and a soft fail (AA24/AA34) each reject exactly that one op with nothing written and no ERC-7562 reputation counter moving (an op that fails first simulation never enters the mempool). The wrapper costs ~7.7k gas per validation and buys a typed rejection reason for simulators, one containment mechanism across all three paths, and independence from any bundler's blame heuristics.
  - Inside the self-call the blob is the entire calldata, so the compiler's `calldatasize` check is effectively a check against the blob's own end: a nested offset can no longer resolve into adjacent userOp fields. Two solc decoder properties to know: its tail check is a SIGNED comparison, so offsets `>= 2^255` do not revert but wrap to a backward pointer and decode garbage (confined to the blob and its 0x44-byte call prefix); and an in-range garbage offset likewise decodes a different struct. Either way the result fails the leaf guards or verification — never a revert, never a consumed leaf, never a pass — fuzzed over the whole nested head-word space in both contracts' validation tests.
...
- **The budget is never a free parameter — including at epoch 0.** `maxSignatures` is decoded from the installed subkey's 68-byte encoding (bytes [64,68)) on BOTH install paths: `initialize` takes the full public-key bundle (validated via `validPublicKey`, so the embedded commitment must recompute) and `rotateStatefulKey` decodes the new subkey. A budget diverging from the key's real tree capacity would misreport `remainingStatefulSignatures()` and desync the on-chain gate from the signer.
...
- **Deploy shape:** impl + OZ `ERC1967Proxy` via sender-guarded CreateX CREATE3 (no in-repo Deployer hop), salt preimages `QUIP:WalletFactory:Impl:V1.0.1-beta.1` (impl) and `QUIP:WalletFactory:Proxy:V1.0.1` (proxy). The salts are load-bearing — CREATE3 ignores initcode, so a stale salt would silently resolve to any pre-existing non-upgradeable factory deployment and skip. Sender-guarding is load-bearing too: the address is a function of (CreateX, `DEPLOY_OPERATOR`, preimage) and only the operator can consume the salt, so the canonical proxy address cannot be squatted on fresh chains.
...
- **Main key: bounded by what it is for, not by a counter.** The main bundle's stateless half is the recovery / handover authority: it signs only in `recoverWallet` / `transferOwnership` rotation contexts, and both of those operations by definition install a full fresh bundle (recovery replaces a lost or burned stateful half; a handover must not leave the old owner holding the recovery root). Under today's paths each main stateless root therefore signs at most once before it is retired — a consequence of those two operations, NOT a one-signature rule: the key is a many-time SPHINCS+-C key and a future path that consumed a stateless signature without replacing the root (e.g. a propose/cancel flow) would be cryptographically fine within the 2^20 budget. What IS load-bearing is that the recovery root's uses stay countable — which is why the stateful-only `rotateKey` consumes a STATEFUL signature (no uncounted stateless use for routine rotation) and why the root must never double as the unmetered ERC-1271 key (next bullet).
- **Re-installation is enforced on-chain:** the stateless root is recorded in `spentStatelessTrees` at install and `recoverWallet` / `transferOwnership` / `migrate` reject any root ever held (`StatelessTreeSpent`) (25). The registry prevents a retired root from coming back under a fresh epoch; it does not — and need not — count signatures.
- **ERC-1271 key isolation is enforced, not assumed.** The recovery root would silently lose its ≤1-use property if the same stateless key also answered routine `isValidSignature` traffic (a view: nothing meters, reverts, or logs). So every path that installs the 1271 key — `initialize`, `migrate`, `setErc1271Key` — receives the FULL 1271 bundle (never a bare commitment, which cannot reveal which root it hides), derives its commitment on-chain, and spends both of its trees in the same lifetime registries as the main key (25). A 1271 bundle that is the main bundle, that carries the main key's stateless root under a fresh stateful subkey (a brand-new commitment over a held recovery root), that reuses any former main or 1271 tree, or that is re-set unchanged, reverts `StatefulTreeSpent` / `StatelessTreeSpent`. In `setErc1271Key` the registry check runs before signature verification, so a rejected bundle consumes no leaf.
...
**Violation consequence:** An ERC-1271 key signing far past 2^20 messages erodes its forgery resistance until approvals become forgeable — silently, with no on-chain symptom. Letting the recovery root answer unmetered traffic (1271, or any view-verified surface) would erode the break-glass authority the same way with no way to count it.
...
**A hash-based tree, once installed on a ShrincsWallet or ShrincsPaymaster, can never be installed on that contract again. Every install path checks and records the tree in an append-only lifetime registry before it becomes current. The registry starts at `-beta.2`, so a contract upgraded in place from `-beta.1` carries exactly one tree the guard cannot see — see "Registry provenance" below.**
...
- **Wallet paths and what they spend:** `initialize` and `migrate` spend all four trees — both halves of the main bundle AND both halves of the dedicated ERC-1271 bundle (main first, so a payload reusing the main bundle or either of its halves for the 1271 role trips on the 1271 bundle); `rotateKey` spends the next stateful tree (the stateless root is carried forward, already spent at install); `recoverWallet` and `transferOwnership` spend both trees of `nextKey` (and `transferOwnership` opens the new epoch's bitmap with the incoming key's acceptance leaf already consumed — 26); `setErc1271Key` spends both trees of the new 1271 bundle before verifying the authorizing stateful signature. The paymaster's `initialize` and `rotateStatefulKey` spend the stateful tree (its stateless half is inert).
- **Migration is strictly fresh on all four halves.** A family round-trip (WOTS+ → SHRINCS → WOTS+ → SHRINCS) leaves earlier SHRINCS trees recorded in this namespace, and `migrate` presents arbitrary bundles — so it is treated like any other install: `StatefulTreeSpent` / `StatelessTreeSpent` on anything ever held, including the current main bundle and the current ERC-1271 bundle (carrying the 1271 key forward across a migration is a re-install).
- **Check-then-record, single primitive.** `_safeInstallStatefulKey` / `_safeInstallStatelessKey` are the only writers; each derives the tree id from the key material itself, reverts if the id is set, and sets it otherwise. No path may read the registry without writing, and nothing ever clears an entry (ERC-7201 append-only slots; upgrades preserve them).
- **Registry provenance — where the on-chain guard does not bind.** The registry is authoritative for every tree installed from `-beta.2` onward, the version that added it. It is never backfilled, and a code-only UUPS upgrade does not record what the old code installed: `upgradeToAndCall(impl, "")` runs no initializer, and `migrate` records only the bundles it is presented with. A contract upgraded in place from `-beta.1` therefore keeps an installed tree the registry has never seen, and on that one tree the guard is absent — rotating back to it, or A→B→A, succeeds and resets the leaf bitmap. This covers the two testnet ShrincsPaymaster proxies that `02_DeployShrincs` moved in place (Base Sepolia and OP Sepolia, DEPLOYMENTS.md) and any `-beta.1` wallet after a code-only upgrade. Base mainnet has no such tree: the whole generation deployed fresh and every install ran under `-beta.2`. Both testnet paymasters held 0 consumed leaves at the moment of upgrade, so no leaf is presently resurrectable. Until an affected contract installs a fresh tree under the new code, the mitigations are operational rather than on-chain: `ShrincsPaymasterClient.rotateStatefulKey` refuses a target tree equal to the installed one before it sends, and `scripts/rotate-shrincs-paymaster-key.mjs` refuses a next derivation index that is not strictly greater than the current one (`ROTATE_ALLOW_NONMONOTONIC=1` relaxes this to a difference check for recovery) as well as a target tree equal to the installed one. Pinned by `test_upgradeInPlace_beta1Provenance_installedTreeStaysUnregistered` in `test/script/Deploy.t.sol`.
- **The ERC-1271 key is in scope.** It is a many-time stateless key by design (23), which is exactly why it must never coincide with — or recycle — a recovery root: its trees are recorded at every install and `setErc1271Key` performs the same check-then-record before anything else. Identity is the tree here too: a fresh commitment over a held stateless root is still a re-install.

**Contracts:** ShrincsWallet, ShrincsPaymaster

**Violation consequence:** Re-installing a spent tree resets its leaf bitmap under a fresh epoch, so every leaf already revealed on-chain becomes valid again — an attacker replays captured signatures (or, having observed a leaf's one-time secret, forges under it) after the owner "rotates" back, and the budget gauge overstates remaining capacity. Skipping the stateless registry would let a recovery root authorize more than one accepted signature, eroding the ≤1-use construction in 23. Leaving the ERC-1271 bundle out of the registry would let the recovery root double as the routine contract-signing key, wearing down the break-glass authority's forgery margin with every dApp signature and no on-chain symptom.

## 26. Shrincs Two-Party Ownership Handover (Recipient Acceptance)

**`ShrincsWallet.transferOwnership` installs `newOwner` and the incoming bundle only if the CURRENT owner authorized the handover (stateful owner-binding + stateless recovery signatures) AND the INCOMING party proved control of both keys it will operate the wallet with: `newOwner`'s signature and a stateful signature from the incoming bundle, each over `(newOwner, nextCommitment)`.**

- **Why.** Every privileged path on the wallet — recovery included — is `onlyOwner` AND SHRINCS-signed. A handover to an address nobody controls, or with a bundle nobody can sign with, therefore strands the wallet with no party able to correct it. The current owner's signatures prove INTENT; they cannot prove the destination is usable. The acceptance does, so a mistyped `newOwner` or an unusable bundle reverts (`InvalidOwnerAcceptance` / `InvalidKeyAcceptance`) instead of completing.
- **Classical half.** `newOwner` signs the wallet's `QuipSignedHash` EIP-712 target (3) of `transferOwnershipPayloadHash(newOwner, nextCommitment)`, verified with `SignatureCheckerLib` (ECDSA for an EOA, ERC-1271 for a contract). The target is shared with the ERC-1271 message surface deliberately: only this wallet's owner ever signs it for this wallet, and the recipient is not the owner until the call lands, so nothing harvestable elsewhere doubles as an acceptance.
- **PQ half.** A stateful signature by the incoming bundle under `ACTION_TRANSFER_OWNERSHIP` with the same payload, verified against the INCOMING commitment (never the installed one) at nonce 0 / epoch 0. It is distinct from the current owner's binding signature by the commitment it is bound to. Its leaf is recorded as used in the NEW epoch at install (`statefulLeavesUsed` starts at 1): the one-time key signed a public message and must never sign again.
- **No live-state binding, by design.** Neither half binds the live nonce or epoch (17). The incoming commitment can be installed on this wallet at most once (25) and both halves bind it together with `newOwner`, so an acceptance authorizes exactly one install of exactly this bundle for exactly this owner on this wallet/chain — and stays valid however many actions the current owner consumes between the recipient signing and the broadcast.
- **Order.** Current-owner authority is established first (stateless rotate, then the stateful binding — which consumes a leaf), then the acceptance is verified, then the registries are written. The acceptance check is view-only; a failed acceptance rolls back the whole call.
- **One handover per bundle (client discipline).** The PQ acceptance is a one-time signature at its leaf; a bundle that accepts two handovers (two wallets, two owners) signs two messages at one leaf. The SDK's `signOwnershipAcceptance` documents this; the current owner's client verifies both halves locally before spending any of its own signatures.
- **Generational.** Deployed V1.0.1-beta.2 wallets are single-step (`handoverAcceptance: "none"` in the SDK's version quirks); the SDK sends them the legacy call.

**Contracts:** ShrincsWallet

**Violation consequence:** A single-step handover with a wrong destination is a total, irreversible loss: the wallet keeps its funds, but every path that could move or recover them is gated behind a party that does not exist, and the party who made the mistake is no longer the one who could fix it.
