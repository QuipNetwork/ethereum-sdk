...
- Revert model: 0.2.0 replaced the library's signature shape checks with revert-as-rejection — garbage signature *internals* (attacker-controlled array lengths inside `userOp.signature` or a 1271 blob) revert inside the verifier instead of returning `0xffffffff`, and the dep explicitly leaves the boolean policy to callers. The wallet's policy boundary is `_tryVerifyStateful`/`_tryVerifyStateless`: every verifier revert maps to "invalid signature", preserving `validateUserOp`'s never-revert-on-bad-sig property and never-revert ERC-1271. Both properties cover signature *content*, not ABI framing: a blob whose nested `PublicKey`/`Signature` tail offsets run past calldata trips Solidity's calldata bounds check (a bare revert) in the wallet's own frame. On the ERC-4337 paths that is deliberate fail-closed behaviour (the EntryPoint maps AA23/AA33 and AA24/AA34 alike to a per-op rejection, no reputation effect); on the ERC-1271 path the re-encode is wrapped in a self-staticcall so it still returns `0xffffffff` (INVARIANTS §19). Accepted trade-off: an inner out-of-gas also reports as an invalid signature. A codeless verifier address stays loud (solc's return-data decoding error is deliberately not swallowed by try/catch) — a missing verifier is never misread as a bad signature.
