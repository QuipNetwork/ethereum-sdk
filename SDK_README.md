...
- ERC-7562: the verifier is storage-free and pure-opcode, so the validation-phase staticcall is compliant (invariant 18 in `INVARIANTS.md`); the trust model is invariant 19.
...
**Sponsorships expire — by default in 15 minutes.** A sponsorship leaf already binds the op's sender, calldata and gas caps, so expiry is the only thing that makes an issued-but-unsubmitted approval go stale on its own: `markLeavesUsed` and `rotateStatefulKey` are explicit owner actions, not expirations. `sponsorUserOp` therefore signs `validUntil = now + validitySeconds` (default `DEFAULT_SPONSORSHIP_VALIDITY_SECONDS`, 15 minutes) unless told otherwise; `validUntil`, `validAfter`, `validitySeconds` and the clock `now` are all overridable, and an unbounded approval is an explicit `validUntil: 0`, never the result of an omitted argument. A malformed window (inverted, already expired, over the 6-byte field) throws `InvalidSponsorshipWindowError` before the verifier is read or a leaf reserved. Size the window to cover the owner's co-signature plus bundler inclusion: a sponsorship that expires unused strands its leaf until you burn it.
